...
export default function Reports() {
  ...
  const [dateRange, setDateRange] = useState({ start: '2024-04-01', end: '2024-04-12' });
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <p className="text-3xl font-bold text-green-600">KES {totalSales.toLocaleString()}</p>
          </div>
          <div className="bg-white shadow rounded-lg p-6">
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <p className="text-3xl font-bold text-purple-600">KES {totalProfit.toLocaleString()}</p>
          </div>
          <div className="bg-white shadow rounded-lg p-6">
            <p className="text-gray-600 text-sm">Avg Transaction</p>
            <p className="text-3xl font-bold text-orange-600">KES {Math.round(totalSales / totalTransactions).toLocaleString()}</p>
          </div>
        </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {topProducts.map((prod) => (
                  <tr key={prod.sku} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{prod.sku}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{prod.name}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{prod.sold}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-green-600 font-semibold">
